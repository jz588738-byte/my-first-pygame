...
class SplitRock(BaseRock):
    def __init__(self, game : 'Game', size = None, center = None):
        super().__init__(game)
    ...
    def split(self, all_sprites, rocks):
        if self.size - 1 >= 0:
            ...
            #分裂石頭
            small_rock1 = SplitRock(self.game, new_size, (self.rect.right, self.rect.centery))
            small_rock2 = SplitRock(self.game, new_size, (self.rect.left, self.rect.centery))
            #加入群組
