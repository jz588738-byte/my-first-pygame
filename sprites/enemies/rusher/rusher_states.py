...
class EntryState(RusherBaseState):
    ...
    def update(self, events=None):
        self.owner.rect.y += self.owner.speed_y
        if self.owner.rect.top >= 0:
            self.owner.rect.top = 0
...
class BurstState(RusherBaseState):
    ...
    def update(self, events=None):
        now = pygame.time.get_ticks()
        #查看玩家是否有經過rusher的方向
        if abs(self.owner.rect.centerx - self.game.player.rect.centerx) < 25:
        ...
        if now - self.start_time >= 1000:
            if self.owner.has_boosted:
                self.owner.rect.y += self.owner.burst_speed_y
            else:
                self.owner.rect.y += self.owner.speed_y
